...
python3 "$E4/generate_correctness_case.py"
...
echo "[4A] Running CODES max-min correctness case"
(
    cd "$CODES_BUILD"
    mpirun -np 1 \
        "$CODES_EXE" \
...
echo "[4A] Running SimGrid max-min correctness case"
"$SIMGRID_EXE" \
